...
def get_text_embedding_mpnet(text: str):
    # todo: also try summarization before BERT
    return sentence_transformer_model_mpnet.encode(text, show_progress_bar=True)

def get_text_embedding_legal_bert(text: str):
    # todo: also try summarization before BERT
    return sentence_transformer_model_legal_bert.encode(text, show_progress_bar=True)
